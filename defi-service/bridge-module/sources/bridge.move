module bridge::bridge {
    ...
    use aptos_std::table;
    use bridge::liquidity_pool;
    use bridge::chains::{Arbitrum, Ethereum, BSC, Polygon, Optimism};
    ...
    public entry fun enable_coin<Coin, Chain>(
        ...
        only_owner(account);
        let settings = borrow_global_mut<BridgeSettings<Chain>>(@bridge);
    ...
    public entry fun setFee<Chain>(token_admin: &signer, _fee_perc: u128) acquires BridgeSettings {
        ...
        let fee_perc = &mut borrow_global_mut<BridgeSettings<Chain>>(@bridge).fee_perc;
        *fee_perc =  _fee_perc;
    }

    public fun get_nonce<Chain>(): u128 acquires BridgeSettings {
        borrow_global<BridgeSettings<Chain>>(@bridge).nonce
    }
    ...
    public entry fun initialize(admin: &signer) acquires RelayersHolder {
        ...
        add_to_relayers_list(admin, @bridge);
    ...
    public entry fun swap_in<CoinType, Chain>(
        ...
        let bridge_settings= borrow_global_mut<BridgeSettings<Chain>>(@bridge);
        assert!(
        ...
        if (type == NATIVE_COIN) {
            ...
            let caps = borrow_global<Caps<CoinType>>(@bridge);
            let mintedCoins = coin::mint<CoinType>(amount, &caps.mint);
            coin::deposit(to, mintedCoins);
        };

        let bridge_events= borrow_global_mut<BridgeEvents<CoinType>>(@bridge);
